...
module.exports = {
   ...
   insertNewForm: async(req, res) =>{
        var sql = `SELECT * FROM CONTENTS_NAME
              WHERE KOR_NAME = ?`;
        var [content, fields] = await (await connection).execute(sql, [req.body.content_name.trim()]);
   ...
   updateForm: async(req, res) =>{
        ...
        var [members_id, fields] = await (await connection).execute(sql, [req.params.form_id]);
        ...
        if(members_id.length > req.body.hero.length){
            ...
            for(let i=0; i<req.body.hero.length - members_id.length; i++){
                var sql = `INSERT FORM_MEMBERS (form_id, hero_id, hero_lv, hero_cho, hero_gak)
                VALUES(?, ?, ?, ?, ?)`;
                var [hero_info, fields] = await (await connection).execute(sql, [req.params.form_id, 0, 0, 0, 0]);
            }
        }
        ...
        for(let i=0; i<req.body.hero.length; i++){
            ...
            var [hero_info, fields] = await (await connection).execute(sql, [req.body.hero[i], lv, cho, gak, req.params.form_id]);
        }
   ...
   insertAnswerForm: async(req, res) =>{
        var sql = `SELECT * from hero_forms
                where id = ?`
        var [req_form, fields] = await (await connection).execute(sql, [req.params.form_id]);
        ...
        for(let i=0; i<req.body.hero.length; i++){
            var sql = `SELECT * FROM HERO_SETTINGS
                WHERE USER_ID = ? AND HERO_ID = ?`;
            var [hero_info, fields] = await (await connection).execute(sql, [req_form[0].user_id, req.body.hero[i]]);
            let lv, cho, gak;
            // console.log(hero_info + typeof(hero_info))
            lv = hero_info.length > 0 ? hero_info[0].lv : 0;
            cho = hero_info.length > 0  ? hero_info[0].cho : 5;
        ...
        var sql = `INSERT FORM_COMMENTS (form_id, author_id, help_form_id, comment_body, last_datetime)
                VALUES(?, ?, ?, ?, ?)`;
        var [comment, fields] = await (await connection).execute(sql, [req.params.form_id, req.user[0].id, form_id, req.body.writer_memo, req.body.last_datetime]);
